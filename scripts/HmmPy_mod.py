...
class HMMparser(object):
    """Parser of --domtblout or --tblout of HMMER
    ...
    def __init__(self, HMMfile):
        ...
        opt_settings = set(self.parameters["Option settings"].split(' '))
        if "--domtblout" in opt_settings:
            self.parameters['type'] = 'domtbl'
            self.matrix = self.domtblParser()
        elif "--tblout" in opt_settings:
            self.parameters['type'] = 'tbl'
            self.matrix = self.tblParser()

    def filterByEvalue(self, evalue=1e-18):
        if self.parameters['type'] == 'domtbl':
            for i, row in enumerate(self.matrix):
                if float(row[12]) > evalue:  # domain[11] -> i-Evalue (whole database)
                    self.matrix.pop(i)
        elif self.parameters['type'] == 'tbl':
            for i, row in enumerate(self.matrix):
    ...
    def filterByCoverage(self, cov=0.35):  # covered fraction of HMM
        if self.parameters['type'] == 'domtbl':
            ...
            print("This type of filter due to technical stuff is only \
                   available for domtbl output, please rerun your hmmer \
                   command specifying --domtblout if you need this filter")
...
if __name__ == "__main__":
    import argparse
    import sys
    usage = """%(prog)s reads domtblout or tblout file and returns a custom \
               filtered result (by Evalue, Bitscore, Coverage of HMM model) \
               or it can show only the Best Bitscore domain for each query \
               on domtblout program output"""
    ...
    parser.add_argument("-e", "--evalue", dest="evalue",
                        help="The maximum E-value threshold to considerate a \
                        domain as a truly hits [be careful with false \
    ...
    parser.add_argument("-u", "--unique", dest="unique", action="store_true",
                        ...
                        query on domtbl program output", default=False)
    parser.add_argument("-s", "--spacer", dest="spacer", help="Select the \
                        spacer for the program output; tab by default", default="\t")
    ...
    if hmm.parameters['type'] == 'domtbl':
        header = ["target name", "accession", "tlen", "query name",
                  "accession", "qlen", "E-value", "score", "bias", "#", "of",
                  "c-Evalue", "i-Evalue", "score__d", "bias_d", "h__from", "h__to",
                  "a__from", "a__to", "e__from", "e__to", "acc", "description of target"]
    elif hmm.parameters['type'] == 'tbl':
        header = ["target name", "accession", "query name", "accession",
                  "E-value", "score", "bias", "E-value__d", "score__d", "bias__d",
                  "exp", "reg", "clu", "ov", "env", "dom", "rep", "inc",
                  "description of target"]
